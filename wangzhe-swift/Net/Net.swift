...
class Net {
    ...
    static let VideoInfo = "https://h5vv.video.qq.com/getinfo"
     ...
     static func GetHero(fail: @escaping ()->Void = {}, success: @escaping ([HeroModel])->Void = { _ in }) -> DataRequest? {
        print("获取英雄数据")
        ...
        CommonData.heros = [HeroModel]()
        return Alamofire.request(MainUrl+HeroList, method: .get).validate().responseJSON(queue: DispatchQueue.global())
        { response in
            switch response.result {
                case .success(let value):
                    let json = JSON(value)
                    for hero in json.arrayValue {
                        CommonData.heros!.append(HeroModel(name: hero["cname"].stringValue,
    ...
    static func GetMing(fail: @escaping ()->Void = {}, success: @escaping ([MingModel])->Void = { _ in }) -> DataRequest? {
        print("获取铭文")
        ...
        CommonData.mings = [MingModel]()
        return Alamofire.request(MainUrl+MingList, method: .get).validate().responseJSON(queue: DispatchQueue.global())
        { response in
            switch response.result {
            case .success(let value):
                let json = JSON(value)
                for ming in json.arrayValue {
                    CommonData.mings!.append(MingModel(id: ming["ming_id"].stringValue,
    ...
    static func GetEquip(fail: @escaping ()->Void = {}, success: @escaping ([EquipModel])->Void = { _ in }) -> DataRequest? {
        print("获取物品")
        ...
        return Alamofire.request(MainUrl+ItemList, method: .get).validate().responseJSON(queue: DispatchQueue.global(), options: .mutableLeaves) { response in
            ...
            case .success(let value):
                ...
                for equip in json.arrayValue {
                    CommonData.items!.append(EquipModel(
    ...
    static func GetSummon(fail: @escaping ()->Void = {}, success: @escaping ([SummonModel])->Void = { _ in }) -> DataRequest? {
        print("获取召唤师技能")
        ...
        CommonData.summons = [SummonModel]()
        return Alamofire.request(MainUrl+SummonerList, method: .get).validate().responseJSON(queue: DispatchQueue.global()) { response in
            ...
            case .success(let value):
                ...
                for summon in json.arrayValue {
                    CommonData.summons!.append(SummonModel(id: summon["summoner_id"].intValue,
    ...
    static func GetHeroInfo(hero: HeroModel, fail: @escaping ()->Void = {}, success: @escaping (HeroInfo)->Void) -> DataRequest? {
        print("获取英雄详情")
