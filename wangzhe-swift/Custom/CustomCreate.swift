...
class CustomCreate {
    ...
    static func showItemTips(item: EquipModel) {
        let width : CGFloat = 5 + 5 + 5 + 60 + 150
        let image = UIImageView()
        image.kf.setImage(with: URL(string: item.imageUrl) , placeholder: self.createActivity())
        let name = UILabel()
     ...
     static func showMingTips(ming: MingModel) {
        let width: CGFloat = 5+5+5+60+150
        let image = UIImageView()
        image.kf.setImage(with: URL(string: ming.imageUrl), placeholder: self.createActivity())
        let name = UILabel()
        name.font = UIFont.systemFont(ofSize: 18)
        name.textColor = CustomColor.mainColor
        name.text = ming.name
